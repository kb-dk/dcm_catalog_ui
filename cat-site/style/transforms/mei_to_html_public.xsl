...
<xsl:stylesheet xmlns="http://www.w3.org/1999/xhtml" 
    ...
    <xsl:variable name="language_pack_file_name">
        <xsl:choose>
            <xsl:when test="$language != ''">
                <xsl:value-of select="string(concat($base_uri, '/style/transforms/language/', $language, '.xml'))"/>
            </xsl:when>
            <xsl:otherwise>
                <xsl:value-of select="string(concat($base_uri, '/style/transforms/language/', $default_language, '.xml'))"/>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:variable>
    ...
    <xsl:template match="m:meiHead/m:workList/m:work" mode="titles">
            ...
            <xsl:for-each select="m:title[@type = 'main' or not(@type)][text()]">
                ...
                <xsl:for-each select="../m:title[@type = 'alternative'][@xml:lang = $lang and text()][position() = $pos]">
                    <h2 class="subtitle alternative_title">
                        <xsl:element name="span"> <xsl:attribute name="class"> <xsl:value-of select="$language_class"/> </xsl:attribute> (<xsl:apply-templates select="."/>) </xsl:element>
                    </h2>
                </xsl:for-each>
            </xsl:for-each>
        </xsl:if>
        ...
        <xsl:for-each select="m:title[@type = 'alternative' and text()]">
            ...
            <xsl:if test="not(../m:title[(@type = 'main' or not(@type)) and text() and @xml:lang = $lang])">
                <xsl:element name="h2">
                    <xsl:element name="span"> <xsl:call-template name="maybe_print_lang"/>(<!--[<xsl:value-of
        ...
        <xsl:if test="m:title[@type = 'original'][text()]">
            ...
            <xsl:element name="h2">
                ...
                <xsl:for-each select="m:title[@type = 'original'][text()]">
                    <xsl:element name="span"> <xsl:call-template name="maybe_print_lang"/> <xsl:value-of select="$l/original_title"/>: <xsl:apply-templates select="."/> </xsl:element>
                    <xsl:call-template name="maybe_print_br"/>
                </xsl:for-each>
            </xsl:element>
        </xsl:if>
            ...
            <xsl:for-each select="m:relation[@rel != '' and normalize-space(substring-after(@label, ':'))]">
                ...
                <xsl:if test="count(preceding-sibling::*[substring-before(@label, ':') = $label]) = 0">
                    ...
                    <div class="list_block">
                        <div class="relation_list">
                            <xsl:if test="$label != ''">
                                <div class="p_heading relation_list_label"> <xsl:value-of select="$label"/>: </div>
                            </xsl:if>
    ...
    <xsl:template match="m:relation" mode="relation_link">
        ...
        <xsl:variable name="href">
            <xsl:choose>
                <xsl:when test="$mermeid_crossref = 'true'">
                    ...
                    <xsl:value-of select="concat('document.xq?n=', encode-for-uri($catalogue_no))"/>
                    <!-- was: -->
        ...
        <!-- Insert link to the related resource with the @label as text -->
        ...
        <xsl:if test="$mermeid_crossref = 'true'">
            <!-- With internal cross-links, add also a link displaying the catalogue number. -->
            <!-- Get collection name and number from linked files -->
            <xsl:variable name="fileName" select="concat($base_file_uri, '/', @target)"/>
            <xsl:variable name="linkedDoc" select="document($fileName)"/>
            <xsl:variable name="file_context" select="$linkedDoc/m:mei/m:meiHead/m:fileDesc/m:seriesStmt/m:identifier[@type = 'file_collection']"/>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <xsl:choose>
                        ...
                        <xsl:otherwise>
                            <p>
                                <span class="label"> <xsl:value-of select="$l/music_incipit"/>: </span>
                            </p>
                        </xsl:otherwise>
                    </xsl:choose>
                    <xsl:apply-templates select="."/>
    ...
    <xsl:template match="m:perfResList">
        ...
        <span class="{$class}">
            ...
            <xsl:if test="m:perfRes[@solo = 'true']">
                ...
                <span class="p_heading:"> <xsl:call-template name="capitalize"> <xsl:with-param name="str"> <xsl:value-of select="$l/soloist"/> </xsl:with-param> </xsl:call-template> <xsl:if test="count(m:perfRes[@solo = 'true']) &gt; 1 and ($language = 'en' or ($language = '' and $default_language = 'en'))">s</xsl:if>: </span>
    ...
    <xsl:template match="m:creation[m:date[text()] or m:geogName[text()]]">
        <xsl:if test="position() = 1">
            ...
            <p> <span class="p_heading"> <xsl:value-of select="$label"/>: </span> <xsl:apply-templates select="m:geogName"/> <xsl:if test="m:date[text()] and m:geogName[text()]"> <xsl:text> </xsl:text> </xsl:if> <xsl:apply-templates select="m:date"/>.</p>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="m:event" mode="performance_details">
        <tr>
            ...
            <td>
                ...
                <xsl:if test="@evidence != ''"> <xsl:variable name="evidence" select="@evidence"/> [<xsl:value-of select="$l/evidence"/>: <xsl:apply-templates select="/m:mei/m:meiHead//*[@xml:id = $evidence]"/>] </xsl:if>
                <xsl:for-each select="m:biblList">
                    <xsl:variable name="no_of_refs" select="count(m:bibl[m:title/text()])"/>
                    <xsl:if test="$no_of_refs &gt; 0">
                        <xsl:choose>
                            <xsl:when test="m:head = 'Reviews' and $no_of_refs = 1"> <br/> <xsl:value-of select="$l/review"/>: </xsl:when>
                            <xsl:otherwise> <br/> <xsl:value-of select="m:head"/>: </xsl:otherwise>
                        </xsl:choose>
            ...
            <xsl:for-each select="m:identifier[text()]">
                <div>
                    ...
                    <xsl:choose>
                        ...
                        <xsl:when test="contains(@label, 'CNU') and contains(@label, 'Source')"> <b> <xsl:apply-templates select="."/> </b>. </xsl:when>
    ...
    <xsl:template match="m:provenance[.//text()]">
        ...
        <xsl:text>: </xsl:text>
        <xsl:for-each select="m:eventList/m:event[*/text()]"> <xsl:for-each select="m:desc"> <xsl:apply-templates/> </xsl:for-each> <xsl:for-each select="m:date[text()]"> <xsl:text> (</xsl:text> <xsl:apply-templates select="."/> <xsl:text>)</xsl:text> </xsl:for-each>. </xsl:for-each>
    </xsl:template>
    ...
    <xsl:template match="m:handList">
        <xsl:if test="count(m:hand[@type = 'main' and (@medium != '' or .//text())]) &gt; 0"> <xsl:if test="m:hand[@type = 'main' and @medium != '']"> <xsl:value-of select="$l/written_in"/> <xsl:text> </xsl:text> </xsl:if> <xsl:for-each select="m:hand[@type = 'main' and (@medium != '' or .//text())]"> <xsl:if test="position() &gt; 1 and position() &lt; last()">, </xsl:if> <xsl:if test="position() = last() and position() &gt; 1"> <xsl:text> </xsl:text> <xsl:value-of select="$l/and"/> <xsl:text> </xsl:text> </xsl:if> <xsl:apply-templates select="." mode="scribe"/> </xsl:for-each>. </xsl:if>
        <xsl:if test="count(m:hand[@type = 'additions' and (@medium != '' or .//text())]) &gt; 0"> <xsl:choose> <xsl:when test="@medium != ''"> <xsl:value-of select="$l/additions_in"/> <xsl:text> </xsl:text> </xsl:when> <xsl:otherwise> <xsl:value-of select="$l/additions"/> <xsl:text> </xsl:text> </xsl:otherwise> </xsl:choose> <xsl:for-each select="m:hand[@type = 'additions']"> <xsl:if test="position() &gt; 1 and position() &lt; last()">, </xsl:if> <xsl:if test="position() = last() and position() &gt; 1"> <xsl:text> </xsl:text> <xsl:value-of select="$l/and"/> <xsl:text> </xsl:text> </xsl:if> <xsl:apply-templates select="." mode="scribe"/> </xsl:for-each>. </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="m:bibl">
        ...
        <xsl:choose>
            ...
            <xsl:when test="(m:genre = 'journal' or m:genre = 'newspaper') and (m:genre = 'article' or m:genre = 'interview')">
                ...
                <xsl:if test="m:title/text()">
                    <xsl:if test="normalize-space(m:title[@level = 'a']) != ''">
                        <xsl:if test="m:author/text()"> <xsl:call-template name="list_authors"/>: </xsl:if>
                    ...
                    <xsl:if test="normalize-space(m:biblScope[@unit = 'page']) != ''">, <xsl:apply-templates select="m:biblScope[@unit = 'page']" mode="pp"/> </xsl:if>
            ...
            <xsl:when test="m:genre = 'letter'">
                <tr>
                    <td class="date_col"> <xsl:apply-templates select="m:creation/m:date[text()]"/> <xsl:if test="m:creation/m:geogName/text() and m:creation/m:date/text()">, </xsl:if> <xsl:apply-templates select="m:creation/m:geogName/text()"/>   </td>
                    <td>
            ...
            <xsl:when test="m:genre = 'manuscript'">
                ...
                <xsl:apply-templates select="m:physLoc[*//text()]"/>
                <xsl:if test="m:creation/m:date//text()"> <xsl:apply-templates select="m:creation/m:date"/>. </xsl:if>
            </xsl:when>
            <xsl:when test="contains(m:genre, 'concert') and contains(m:genre, 'program')">
                ...
                <xsl:for-each select="m:imprint[*//text()]"> <xsl:if test="m:publisher/text()"> <xsl:apply-templates select="m:publisher"/> <xsl:if test="m:pubPlace//text() or m:date//text()">, </xsl:if> </xsl:if> <xsl:value-of select="m:pubPlace"/> <xsl:if test="m:date/text()"> <xsl:text> </xsl:text> <xsl:apply-templates select="m:date[text()]"/> </xsl:if>. </xsl:for-each>
                <xsl:call-template name="hosts"/>
                <xsl:apply-templates select="m:ptr"/>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="m:imprint[*//text()]">
        ...
        <xsl:if test="m:publisher/text()"> <xsl:apply-templates select="m:publisher"/>: </xsl:if>
    ...
    <xsl:template match="m:author">
        <xsl:value-of select="."/>
        <xsl:if test="@type and @type != ''"> <xsl:text> </xsl:text>(<xsl:value-of select="@type"/>)</xsl:if>
    </xsl:template>
                ...
                <xsl:choose>
                    <xsl:when test="$number_of_issues &gt; 0">
                        <xsl:for-each select="m:biblScope[@unit = 'issue' and text()]">
                            ...
                            <xsl:value-of select="."/>
                            <xsl:if test="../m:biblScope[@unit = 'page'][position()]/text()">, <xsl:apply-templates select="../m:biblScope[@unit = 'page'][position()]" mode="pp"/> </xsl:if>
                        </xsl:for-each>
                    </xsl:when>
                    <xsl:otherwise>
        ...
        <xsl:choose>
            <xsl:when test="string-length($date) = 10">
                ...
                <xsl:choose>
                    ...
                    <xsl:when test="(string(number($year)) != 'NaN' or string($year) = '????' or (string(number(substring($year, 1, 2))) != 'NaN' and substring($year, 3, 2) = '??')) and (string(number($month)) != 'NaN' or string($month) = '??') and (string(number($day)) != 'NaN' or string($day) = '??') and substring($date, 5, 1) = '-' and substring($date, 8, 1) = '-'"> <xsl:choose> <xsl:when test="substring($day, 1, 1) = '0'"> <xsl:value-of select="substring($day, 2, 1)"/> </xsl:when> <xsl:otherwise> <xsl:value-of select="$day"/> </xsl:otherwise> </xsl:choose>.<xsl:choose> <xsl:when test="substring($month, 1, 1) = '0'"> <xsl:value-of select="substring($month, 2, 1)"/> </xsl:when> <xsl:otherwise> <xsl:value-of select="$month"/> </xsl:otherwise> </xsl:choose>.<xsl:value-of select="$year"/> </xsl:when>
                    <xsl:otherwise>
                        <xsl:apply-templates/>
                    </xsl:otherwise>
                </xsl:choose>
    ...
    <xsl:template match="text()[contains(., '&amp;nbsp;')] | @*[contains(., '&amp;nbsp;')]" mode="entities" priority="1"> <xsl:apply-templates select="local:nodifier(substring-before(., '&amp;nbsp;'))" mode="entities"/> <xsl:apply-templates select="local:nodifier(substring-after(., '&amp;nbsp;'))" mode="entities"/> </xsl:template>

    <xsl:template match="text()[contains(., '&amp;lt;')] | @*[contains(., '&amp;lt;')]" mode="entities" priority="2"> <xsl:apply-templates select="local:nodifier(substring-before(., '&amp;lt;'))" mode="entities"/>&lt;<xsl:apply-templates select="local:nodifier(substring-after(., '&amp;lt;'))" mode="entities"/> </xsl:template>

    <xsl:template match="text()[contains(., '&amp;gt;')] | @*[contains(., '&amp;gt;')]" mode="entities" priority="3"> <xsl:apply-templates select="local:nodifier(substring-before(., '&amp;gt;'))" mode="entities"/>&gt;<xsl:apply-templates select="local:nodifier(substring-after(., '&amp;gt;'))" mode="entities"/> </xsl:template>
